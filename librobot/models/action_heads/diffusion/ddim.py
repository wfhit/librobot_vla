...
class DDIMActionHead(DDPMActionHead):
    ...
    def sample(
        self, embeddings: torch.Tensor, temperature: float = 1.0, eta: float = 0.0, **kwargs
    ) -> torch.Tensor:
        ...
        for i, t in enumerate(timesteps):
            ...
            pred_x0 = (actions - torch.sqrt(1 - alpha_cumprod_t) * predicted_noise) / torch.sqrt(
                alpha_cumprod_t
            )
            ...
            noise = (
                torch.randn_like(actions) if i < len(timesteps) - 1 else torch.zeros_like(actions)
            )
