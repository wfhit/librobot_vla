...
graph TB
    ...
    subgraph WIFI["📡 WiFi 7 (IEEE 802.11be)"]
        direction TB
        W7["Multi-Link Operation (MLO)\nUp to 46 Gbps Peak Throughput\n320 MHz Channel Bandwidth\n4096-QAM Modulation\n< 2 ms Latency"]
    end

    subgraph VEHICLE["🚜 Wheel Loader (On-Vehicle)"]
        direction TB
        subgraph ORIN["NVIDIA Jetson Orin NX\n8 GB / 70 TOPS\nProxy Node"]
...
WiFi 7 is selected for the wireless link between the HPC and the on-vehicle proxy due to its ultra-low latency and high throughput characteristics.
...
| **Role** | Proxy: camera capture, image preprocessing, WiFi 7 uplink, action command relay |
| **Camera Interfaces** | USB 3.0 (OAK-D + ZED 2i) |
...
| Camera | Key Features |
|---|---|
| **Luxonis OAK-D** | Stereo depth + RGB, on-device neural inference (Myriad X VPU), USB 3.0 |
