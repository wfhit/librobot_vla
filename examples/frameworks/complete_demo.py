...
def main():
    ...
    from librobot.models.frameworks import (
        GR00TVLA, Pi0VLA, OctoVLA, OpenVLA,
        RT2VLA, ACTVLA, HelixVLA, CustomVLA
    )
        ...
        try:
            ...
            with torch.no_grad():
                if name == 'ACT':
                    ...
                    pred = model.predict_action(data['images'], None, data['proprioception'], data['task_ids'])
                else:
                    pred = model.predict_action(data['images'], data['text'], data['proprioception'])
